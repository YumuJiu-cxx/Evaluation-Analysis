...
print('cat类和对应的id:')
print(df['cat'].factorize()[0])
print(df['cat'].factorize()[1])

cat_id_df = df[['cat', 'cat_id']].drop_duplicates().sort_values('cat_id').reset_index(drop=True)  # 去重
cat_to_id = dict(cat_id_df.values)  # 建立字典{'书籍': 0, }
id_to_cat = dict(cat_id_df[['cat_id', 'cat']].values)  # 建立字典{0: '书籍', }
print(df[['cat', 'cat_id']], '\n')

np.save('idx_to_label/idx_to_label.npy', id_to_cat)  # 保存词库对应表
...
# 保存tokenizer
with open('Tokenizer/tokenizer_classify.pickle', 'wb') as handle:
...
# 保存模型
model.save('Model/comment-classify_model.h5')
